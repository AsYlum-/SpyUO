...
namespace Ultima.Package
{
    ...
    public class UltimaAnimation
    {
        ...
        /// Gets frames.
        /// </summary>
        public List<UltimaAnimationFrame> Frames { get; set; }
        #endregion
        ...
        public UltimaAnimation(BinaryReader reader, bool legacy)
        {
            ...
            // Read frame table
            Frames = new List<UltimaAnimationFrame>(frameCount);

            for (int i = 0; i < frameCount; i++)
                Frames.Add(new UltimaAnimationFrame(reader));

            // Read pixel data
            if (legacy)
            {
                for (int i = 0; i < frameCount; i++)
                    Frames[i].ReadLegacyPixelData(reader, palette, frameTableAddress);
            }
            else
            {
                for (int i = 0; i < frameCount; i++)
                    Frames[i].ReadPixelData(reader, palette, frameTableAddress);
            }
        }
        #endregion
    ...
    public class UltimaAnimationFrame
    {
        ...
        private int FrameOffset { get; set; }
        #endregion
        ...
        public UltimaAnimationFrame(BinaryReader reader)
        {
            ...
            StartX = reader.ReadInt16();
            StartY = reader.ReadInt16();
            Width = reader.ReadInt16() - StartX;
            Height = reader.ReadInt16() - StartY;
        ...
        {
            reader.BaseStream.Seek(frameTableStart + FrameOffset, SeekOrigin.Begin);
            ...
            while (index < size)
            {
                ...
                else
                {
                    ...
                    if ((factor & 0xF0) > 0)
                    {
                        ...
                        PixelData[index++] = (byte)((color & 0xFF) >> 0);
                        PixelData[index++] = (byte)((color & 0xFF00) >> 8);
                        PixelData[index++] = (byte)((color & 0xFF0000) >> 16);
                        PixelData[index++] = (byte)((color & 0xFF000000) >> 24);
                    }
